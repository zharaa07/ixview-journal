...
(function () {
    ...
    function pdLoadSettings() {
        ...
        // Valid drawdown display modes: R, % or $.
        Object.keys(merged.drawdown).forEach(function (k) {
            if (["R", "%", "$"].indexOf(merged.drawdown[k]) === -1) merged.drawdown[k] = "%";
        });
        return merged;
    }
    ...
    function pdSaveSettings() {
        ...
        if (window.cloudSaveField) window.cloudSaveField("pdDashboardSettings", pdSettings);
    }
    ...
    //
    // Rebuilt on a single, consistent percentage-based equity engine (see
    // journal.js / model.js buildEquitySeries for the same algorithm — equity
    // compounds from each trade's Return %, drawdown % is always measured
    // against the true running equity peak, never approximated from R or
    // trade counts).
    //
    // Cards:
    //   Current Drawdown  — distance from current equity to the most recent peak
    //   Max Drawdown       — largest peak-to-trough decline ever recorded
    //   Average Drawdown   — average of completed (recovered) drawdown episodes
    //   Daily Drawdown      — decline from today's starting equity to today's low

    function pdBuildEquitySeries(sortedTrades) {
        ...
        sortedTrades.forEach(function (t) {
            const r = (typeof t.returnPct === "number" && isFinite(t.returnPct)) ? t.returnPct : 0;
            ...
            if (drawdownPct > 0) {
                if (!currentEpisode) currentEpisode = { maxDrawdownPct: drawdownPct };
                else if (drawdownPct > currentEpisode.maxDrawdownPct) currentEpisode.maxDrawdownPct = drawdownPct;
            }
            ...
            points.push({
                ...
                pnlUSD: (t.pnlUSD !== null && t.pnlUSD !== undefined) ? t.pnlUSD : 0
            });
        });

        return { points: points, episodes: episodes };
    }

    // R-based drawdown engine (restored) — mirrors the % engine but tracks the
    // running cumulative Result (R) instead of compounding Return %, exactly
    // like the app's other R-based stats (e.g. Total R).
    function pdBuildRSeries(sortedTrades) {
        let equity = 0, peak = 0;
        const points = [];
        const episodes = [];
        let currentEpisode = null;
        ...
        sortedTrades.forEach(function (t) {
            const r = (typeof t.resultR === "number" && isFinite(t.resultR)) ? t.resultR : 0;
            equity += r;
            ...
            const drawdownR = peak - equity;
            if (drawdownR > 0) {
                if (!currentEpisode) currentEpisode = { maxDrawdownR: drawdownR };
                else if (drawdownR > currentEpisode.maxDrawdownR) currentEpisode.maxDrawdownR = drawdownR;
            }
    ...
    function pdComputeDrawdowns(filteredSorted) {
        const series = pdBuildEquitySeries(filteredSorted);
        ...
        const averageDrawdownPct = series.episodes.length
            ? series.episodes.reduce(function (s, e) { return s + e.maxDrawdownPct; }, 0) / series.episodes.length
            : null; // null = no completed (recovered) drawdown episode yet

        // Daily Drawdown: decline from *today's* starting equity to today's lowest
        // point — the standard prop-firm "daily loss" definition — not a
        // reset-to-zero pseudo metric.
        const todayKey = new Date().toISOString().slice(0, 10);
        let dailyStartEquity = 100;
        for (let i = 0; i < points.length; i++) {
            if (points[i].day === todayKey) break;
            dailyStartEquity = points[i].equity;
        }
        let dailyTrough = dailyStartEquity;
        let dailyUSD = 0;
        points.forEach(function (p) {
            if (p.day === todayKey) {
                if (p.equity < dailyTrough) dailyTrough = p.equity;
                dailyUSD += p.pnlUSD;
            }
        });
        const dailyDrawdownPct = dailyStartEquity > 0
            ? Math.max(0, ((dailyStartEquity - dailyTrough) / dailyStartEquity) * 100)
            : 0;

        // $-mode reference values (informational — based on the manually entered
        // "Profit/Loss in USD" field, independent of the % equity engine).
        let usdRunning = 0, usdRunningPeak = 0, usdCurrentDD = 0, usdMaxDD = 0;
        filteredSorted.forEach(function (t) {
            usdRunning += (t.pnlUSD || 0);
            if (usdRunning > usdRunningPeak) usdRunningPeak = usdRunning;
            const dd = usdRunningPeak - usdRunning;
            if (dd > usdMaxDD) usdMaxDD = dd;
        });
        usdCurrentDD = usdRunningPeak - usdRunning;

        // R-mode values (restored)
        const rSeries = pdBuildRSeries(filteredSorted);
        const rPoints = rSeries.points;
        const currentDrawdownR = rPoints.length ? rPoints[rPoints.length - 1].drawdownR : 0;
        const maxDrawdownR = rPoints.reduce(function (m, p) { return Math.max(m, p.drawdownR); }, 0);
        const averageDrawdownR = rSeries.episodes.length
            ? rSeries.episodes.reduce(function (s, e) { return s + e.maxDrawdownR; }, 0) / rSeries.episodes.length
            : null;

        let dailyStartR = 0;
        for (let i = 0; i < rPoints.length; i++) {
            if (rPoints[i].day === todayKey) break;
            dailyStartR = rPoints[i].equity;
        }
        let dailyTroughR = dailyStartR;
        rPoints.forEach(function (p) {
            if (p.day === todayKey && p.equity < dailyTroughR) dailyTroughR = p.equity;
        });
        const dailyDrawdownR = Math.max(0, dailyStartR - dailyTroughR);

        return {
            current: { pct: currentDrawdownPct, usd: usdCurrentDD, r: currentDrawdownR },
            max: { pct: maxDrawdownPct, usd: usdMaxDD, r: maxDrawdownR },
            average: { pct: averageDrawdownPct, usd: null, r: averageDrawdownR },
            daily: { pct: dailyDrawdownPct, usd: dailyUSD < 0 ? Math.abs(dailyUSD) : 0, r: dailyDrawdownR }
        };
    }

    function pdFormatDrawdownValue(entry, mode) {
        if (mode === "R") {
            if (entry.r === null || entry.r === undefined) return "—";
            return entry.r.toFixed(2) + "R";
        }
        if (mode === "$") {
            if (entry.usd === null || entry.usd === undefined) return "—";
            return "$" + Math.abs(entry.usd).toFixed(0);
        }
        if (entry.pct === null || entry.pct === undefined) return "—";
        return entry.pct.toFixed(2) + "%";
    }
    ...
    function pdRenderDrawdownCards(filteredSorted) {
        ...
        const cards = [
            { key: "current", label: "Current Drawdown" },
            { key: "max", label: "Max Drawdown" },
            { key: "average", label: "Average Drawdown" },
            { key: "daily", label: "Daily Drawdown" }
        ];

        grid.innerHTML = cards.map(function (c) {
            const mode = pdSettings.drawdown[c.key] || "%";
            const entry = dd[c.key];
            const isNA = (entry[mode === "R" ? "r" : mode === "$" ? "usd" : "pct"] === null || entry[mode === "R" ? "r" : mode === "$" ? "usd" : "pct"] === undefined);
            const note = isNA
                ? '<div style="font-size:9.5px;color:var(--text-tertiary);margin-top:2px;">No completed (recovered) drawdown yet</div>'
            ...
            return '<div class="pd-card pd-drawdown-card">' +
                ...
                '</div>' +
                '<div class="pd-drawdown-value">' + pdFormatDrawdownValue(entry, mode) + '</div>' +
                note +
