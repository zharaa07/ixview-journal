...
function drawSessionChart() {
    ...
    //
    // Root cause of the original bug: "session" is a value the trader picks
    // manually on the Add Trade form (Asia / London / New York AM / New York
    // PM) — it is never derived from the trade's date/time, so there is no
    // timezone or DST logic involved at all. Two real issues caused sessions
    // to go missing: (1) the ranking list explicitly filtered out any session
    // with 0 trades, hiding it entirely; (2) "New York AM" and "New York PM"
    // are two distinct stored values that never matched a plain "New York"
    // bucket, and "Sydney" was never even an available option to select.
    // Fixed below: Sydney was added as a selectable session on the Add Trade
    // form, and AM/PM are merged into one "New York" bucket for this analysis
    // (the underlying stored trade data is untouched either way).
    const SESSION_ORDER = ["Sydney", "Asia", "London", "New York"];

    function canonicalSessionOf(rawSession) {
        if (!rawSession) return null;
        if (rawSession === "New York AM" || rawSession === "New York PM" || rawSession === "New York") return "New York";
...
t.result,
t.resultR,
t.returnPct !== undefined && t.returnPct !== null ? t.returnPct + "%" : "",
