...
(function () {
    ...
    function pdLoadSettings() {
        ...
        // "R" mode is retired — the drawdown engine is percentage-based now.
        Object.keys(merged.drawdown).forEach(function (k) {
            if (merged.drawdown[k] !== "%" && merged.drawdown[k] !== "$") merged.drawdown[k] = "%";
        });
        return merged;
    }
    ...
    function pdComputeDrawdowns(filteredSorted) {
        ...
        return {
            current: { pct: currentDrawdownPct, usd: usdCurrentDD },
            max: { pct: maxDrawdownPct, usd: usdMaxDD },
            average: { pct: averageDrawdownPct, usd: null },
            daily: { pct: dailyDrawdownPct, usd: dailyUSD < 0 ? Math.abs(dailyUSD) : 0 }
        };
    }
    ...
    function pdRenderDrawdownCards(filteredSorted) {
        ...
        grid.innerHTML = cards.map(function (c) {
            ...
            const isNA = (mode === "%" && (entry.pct === null || entry.pct === undefined));
            const note = isNA
